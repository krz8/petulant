...
used to process command-lines as described by the keyword arguments.
This returned function takes one mandatory argument and two keyword
arguments of its own.  The mandatory argument is a function that is
...
- :WINDOWS Disregard the current running system, and process the
  ...
  (with-context-simple (argopts flagopts aliases styles)
	...
	(labels ((wrap (x y z) (funcall fn x (or (and (eq :opt x)
						      (funcall keyify-fn y))
						 y)
					z)))
	  (funcall parser
...
options (aka switches) and arguments are recognized.  PROCESS is
mostly meant for \"one time\" parsing as it creates a new command line
processor on eveery invocation; if you have many command-lines to
process, you are better served by MAKE-PROCESSOR.
