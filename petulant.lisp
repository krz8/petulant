...
(defun parse-windows-cl (arglist fn
			 &optional
...
PARSE-WINDOWS-CLI works through ARGLIST, a flat list of strings from a
command line, parsing it according to most Windows behaviors.  As
switches are identified, SWARGP-FN is called to determine if that
...
a switch should take the next word in ARGLIST as an argument.  The
default binding of SWARGP-FN always returns NIL, indicating that any
ambiguous switch is assumed not to take an argument.  A non-ambiguous
switch with an argument is one that uses the colon character (e.g.,
\"/foo:bar\").
