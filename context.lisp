...
(defclass context ()
  ((appname :initarg :appname :initform "" :accessor appname
   ...
   (argv :initarg :argv :initform nil :accessor argv
...
(defun make-context-simple (argopts flagopts aliases styles argv)
...
Finally, ARGV is a list of strings that should be used instead of the
...
(defmacro with-context-simple ((argopts flagopts aliases style argv)
			       &body body)
...
command-line\) to a value that can be used to index internal data
structure during option processing.  The presence of :UP or :DOWN in
the styles list of the current *CONTEXT* triggers case conversion on
the option string."
  (cond
...
(defun optname2-fn ()
  "Returns a function that rewrites a supplied option, as already
renamed by OPTNAME1-FN, into a value to be returned to a caller.  The
presence of :KEY in the styles list of the current *CONTEXT* triggers
conversion from string to a keyword symbol."
  (cond
    ((stylep :key) (lambda (string) (intern string "KEYWORD")))
    (t             #'identity)))

(defun optname-fn ()
  "Returns a function that rewrites a supplied option string, as found
on a command-line, into a value \(string or keyword\) according to the
presence of :UP, :DOWN, and/or :KEY in the current *CONTEXT*.  This is
simply a composition of the results of OPTNAME1-FN and OPTNAME2-FN."
  (compose (optname2-fn) (optname1-fn)))
